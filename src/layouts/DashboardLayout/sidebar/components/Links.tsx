...
const SidebarLinks = ({ onClickRoute }: Props) => {
  ...
  const createLinks = (routes: any) => {
    const managementRoutes = routes.filter((route: { path: string }) => route.path === "orders" || route.path === "products");
    const interiorRoutes = routes.filter((route: { path: string }) => route.path !== "orders" && route.path !== "products" && route.path !== "create");
            ...
            <Link key={`route-${routeIndex}`} href={route.path} onClick={(e) => handleRouteClick(routeIndex, e)}>
              ...
              >
                <li className="my-[3px] flex cursor-pointer items-center px-8">
                  <span className={`${activeIndex === routeIndex ? "font-bold text-blue-500 dark:text-white" : "font-medium text-gray-400"}`}>
                    {route.icon ? route.icon : <DashIcon />}{" "}
                  </span>
                  <p className={`leading-1 ml-4 flex ${activeIndex === routeIndex ? "font-medium text-blue-500 dark:text-white" : "font-medium text-gray-400"}`}>
                    {route.name}
                  </p>
                </li>
              </motion.div>
